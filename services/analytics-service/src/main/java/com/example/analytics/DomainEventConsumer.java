...
public class DomainEventConsumer {
    ...
    public void consume(ConsumerRecord<String,String> record) throws Exception {
        ...
        JsonNode payload = unwrap(json.readTree(record.value()));
        ...
        int inserted = db.sql("insert into bronze.domain_events(event_id,domain,event_type,payload,occurred_at) values(:id,:domain,:type,cast(:payload as jsonb),:at) on conflict do nothing")
            .param("id", id).param("domain", domain).param("type", eventType).param("payload", payload.toString()).param("at", sqlTimestamp(occurredAt)).update();
    ...
    private void invoice(JsonNode p, Instant at) {
        db.sql("insert into silver.invoices values(:id,:customer,:amount,:currency,:status,:at) on conflict do nothing").param("id",uuid(p,"id")).param("customer",uuid(p,"customerId")).param("amount",decimal(p,"amount")).param("currency",text(p,"currency")).param("status",text(p,"status")).param("at",sqlTimestamp(at)).update();
        refreshInvoiceNetAmount();
    }
    private void adjustment(JsonNode p, Instant at) {
        db.sql("insert into silver.invoice_adjustments values(:id,:invoice,:amount,:currency,:reason,:at) on conflict do nothing").param("id",uuid(p,"id")).param("invoice",uuid(p,"invoiceId")).param("amount",decimal(p,"amount")).param("currency",text(p,"currency")).param("reason",text(p,"reason")).param("at",sqlTimestamp(at)).update();
        refreshInvoiceNetAmount();
    }
    ...
    private void payment(JsonNode p, Instant at) {
        db.sql("insert into silver.payments values(:id,:invoice,:amount,:currency,:status,:at) on conflict do nothing").param("id",uuid(p,"id")).param("invoice",uuid(p,"invoiceId")).param("amount",decimal(p,"amount")).param("currency",text(p,"currency")).param("status",text(p,"status")).param("at",sqlTimestamp(at)).update();
    ...
    private void identity(JsonNode p, Instant at) {
        db.sql("insert into silver.identities values(:id,:customer,:login,:status,:at) on conflict do nothing").param("id",uuid(p,"id")).param("customer",uuid(p,"customerId")).param("login",text(p,"login")).param("status",text(p,"status")).param("at",sqlTimestamp(at)).update();
    }
    ...
    private java.sql.Timestamp sqlTimestamp(Instant value){return java.sql.Timestamp.from(value);}
